...
Transformer::Transformer(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
  ...
  nh_private_.param("timestamp_tolerance_sec", timestamp_tolerance_sec, timestamp_tolerance_sec);
  timestamp_tolerance_ns_ = static_cast<int64_t>(timestamp_tolerance_sec * kNanoSecondsInSecond);

  // Transform settings.
  nh_private_.param("use_tf_transforms", use_tf_transforms_, use_tf_transforms_);
  nh_private_.param("use_transforms_from_file", use_transforms_from_file_,
                    use_transforms_from_file_);
  ...
  if (use_transforms_from_file_) {
    std::string transforms_file;
    nh_private_.param("transforms_file", transforms_file, transforms_file);
    std::vector<double> data(8);
    std::ifstream myReadFile;
    myReadFile.open(transforms_file);
    while (!myReadFile.eof()) {
      for (int col = 0; col < 8; ++col) {
        myReadFile >> data[col];
      }
      geometry_msgs::TransformStamped transform_msg;
      transform_msg.header.stamp = ros::Time(data[0]);
      transform_msg.transform.translation.x = data[1];
      transform_msg.transform.translation.y = data[2];
      transform_msg.transform.translation.z = data[3];
      transform_msg.transform.rotation.x = data[4];
      transform_msg.transform.rotation.y = data[5];
      transform_msg.transform.rotation.z = data[6];
      transform_msg.transform.rotation.w = data[7];
      transform_queue_.push_back(transform_msg);
    }
  } else if (!use_tf_transforms_) {
    transform_sub_ = nh_.subscribe("transform", 40, &Transformer::transformCallback, this);
...
void Transformer::transformCallback(const geometry_msgs::TransformStamped& transform_msg) {
  transform_queue_.push_back(transform_msg);
}
...
bool Transformer::lookupTransformQueue(const ros::Time& timestamp, Transformation* transform) {
  CHECK_NOTNULL(transform);
  ...
  if (match_found) {
    ...
    geometry_msgs::TransformStamped transformStamped = *it;
